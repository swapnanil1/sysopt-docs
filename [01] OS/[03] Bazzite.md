...
| 1 | [Update & rebase to DX](./bazzite/01-update-rebase-dx.md) — first boot: update, rebase to `bazzite-dx`, Docker, node/php/laravel/go via brew |
| 2 | [fstab](./bazzite/02-fstab.md) — root subvolumes on a HDD, games HDD under `/var/mnt` |
...
Flow: `0 (installer shell) → first boot → 1 update + DX → 2 → 3 → 4 → 5 → 6 → 7 → 8 → reboot → 9 pin → 10 apps`. Kargs and layered packages only take effect after a reboot; batch them.
